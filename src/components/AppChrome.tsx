...
import { ChapterDrawer, ChapterMenuButton, type ProgressState } from "./ChapterNavigator";
import { ReadNavLink } from "./ReadNavLink";

export function AppChrome({ children, userEmail, sectionProgress = {} }: { children: ReactNode; userEmail: string; sectionProgress?: Record<string, ProgressState | undefined> }) {
  const courseChapters = getChapters();
  const chapters = useMemo(() => courseChapters.map((chapter) => ({ slug: chapter.slug, title: chapter.title, sectionIds: chapter.sections.map((section) => section.id) })), [courseChapters]);
  const pathname = usePathname();
  const currentSlug = useMemo(() => pathname.match(/^\/course\/([^/?#]+)/)?.[1] ?? chapters[0]?.slug ?? "", [chapters, pathname]);
  const showChapterMenu = pathname.startsWith("/course/");
  const [chapterDrawerOpen, setChapterDrawerOpen] = useState(false);

  return (
    <div className="min-h-screen overflow-x-clip bg-[#f6f0e7] text-stone-950">
      <header className="sticky top-0 z-40 border-b border-stone-200/80 bg-[#f6f0e7]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-2.5 sm:gap-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-2">
            ...
            {showChapterMenu ? <ChapterMenuButton onClick={() => setChapterDrawerOpen(true)} expanded={chapterDrawerOpen} /> : null}
          </div>
        ...
        <nav className="mx-auto grid max-w-7xl grid-cols-3 gap-2 px-4 pb-2 text-center text-sm font-semibold md:hidden">
      ...
      <ChapterDrawer key={currentSlug} chapters={chapters} currentSlug={currentSlug} initialProgress={sectionProgress} open={chapterDrawerOpen} onClose={() => setChapterDrawerOpen(false)} />
      {children}
    </div>
  );
}
