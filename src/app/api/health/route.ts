...
const requiredTables = ["notes", "progress", "recall_attempts", "recall_prompts", "transcription_jobs"];
...
export async function GET() {
  ...
  try {
    ...
    const tables = await db.execute<{ table_name: string }>(
      sql`select table_name from information_schema.tables where table_schema = 'public' and table_name in ('notes', 'progress', 'recall_attempts', 'recall_prompts', 'transcription_jobs') order by table_name`,
    );
